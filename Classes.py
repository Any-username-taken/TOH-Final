...
class Monster:
    def __init__(self, name, type1, type2, max_health, defense, base_dmg, dodge_chance, willpower, mana, move_set,
                 item_drop, gold):
        self.name = f'\033[94m{name.title()}\033[0m'
        self.type = type1
        self.type2 = f"{type2}"
    ...
    def show_health(self):
        ...
        type_n_name = self.type2
        type_n_name = type_n_name + self.name
        type_n_name = type_n_name.strip() + " "
        self.type = list(self.type)
        if isinstance(self.type, tuple) or isinstance(self.type, list) and len(self.type) > 1:
            type_ = ', '.join(self.type).strip()
        else:
            type_ = ''.join(self.type)
        if len(self.inflicted) > 1:
            conjoined = ', '.join(self.inflicted)
            status = f'status: \033[51m{conjoined}\033[0m'
        elif len(self.inflicted) == 1:
            status = f'status: \033[51m {str(self.inflicted[0])} \033[0m'
        else:
            status = ''
        print(health_text.format(name=type_n_name, inflicted=status, type=type_, health_bar=("\033[51m\033[102m"+str(ch1) +
                                                                                             '\033[101m' + str(ch2) +
                                                                                             '\033[0m')))
    ...
    def heal(self, amount):
        if amount + self.h >= self.mh:
            print(self.name, "\033[92mFully healed!\033[0m")
            self.h = self.mh
        else:
            print(self.name, "healed\033[92m", str(amount), "\033[0mHealth!")
            self.h += amount

    def max_increase(self, amount):
        self.mh += amount
        print(self.name, "increased their \033[93mmax health\033[0m!")
        time.sleep(1)
        self.heal(amount)
    ...
    def activate_status(self, receive, turns):
        ...
        if receive not in self.inflicted:
            print(self.name, 'was inflicted with\033[91m', receive + '\033[0m!')
    ...
    def inflict_status(self):
        ...
        if len(self.inflicted) > 0:
            ...
            for i in range(len(self.inflicted)):
                for n in range(len(self.inflicted)):
                    try:
                        if self.last_turn[n] < 1:
                            self.last_turn.pop(n)
                            self.inflicted.pop(n)
                            self.increase_dmg.pop(n)
                    except IndexError:
                        pass
    ...
    def effect_type(self, num):
        if self.inflicted[num] == 'burn' or self.inflicted[num] == 'hells inferno':
            if self.will < 15 and self.inflicted[num] == "burn":
            ...
            if 'dark' in self.type or 'undead' in self.type:
                ...
                self.heal(2)
                self.will += 5
                print(self.name + ' felt rejuvenated!')

        elif self.inflicted[num] == 'curse of darkness':
            if 'dark' in self.type or 'undead' in self.type:
                self.temp_buff_d += int(self.d / 2) + 1
                self.will += 5
            ...
            if self.last_turn[num] < 2:
                if not "death counter" in self.inflicted:
                    time.sleep(1)
            ...
            self.temp_buff_d += 2.5

        elif self.inflicted[num] == "strengthen II":
            self.temp_buff_d += 2.5

        elif self.inflicted[num] == "strengthen III":
            self.temp_buff_d += 3.5

        elif self.inflicted[num] == "regen":
            self.heal(4)

        elif self.inflicted[num] == "def UP":
            self.temp_def += 5
    ...
    def raw_dmg(self, damage, position, can_die, affect_will):
        txt = '{name} took \033[91m{dmg}\033[0m damage from \033[91m{inflicted}\033[0m!'
        if isinstance(damage, list):
            if self.h - damage[0] < 1 and can_die == 'yes':
                self.h = 0
    ...
    def take_health(self, amount):
        self.h -= amount
        print(self.name, 'took\033[91m', amount, '\033[0mdamage!')
        if self.h < 1 and self.will > 100 and self.survive == 0:
    ...
    def receive_dmg(self, attacker, amount):
        ...
        if self.dc >= dodge:
            print(self.name, '\033[96mevaded\033[0m the attack!')
        else:
            # basic dmg (cause my original idea didn't work)
    ...
    def cal_total_dmg(self, target):
        if self.temp_buff_d > 0:
            temp_d = (self.temp_dmg + self.bd) * self.temp_buff_d
        else:
        ...
        if target.type[0] in self.type_weak:
            temp_d = int(temp_d / 2)
            print("The attack \033[94mwasn't very effective\033[0m...")

        if self.type[0] in target.type_weak:
            temp_d = temp_d * 2
            print("The attack was \033[93msuper effective!\033[0m")
    ...
    def enemy_move(self, target):
        ...
        if not move[2] == "none":
            ran = random.randint(1, int(move[2].split("-")[1]))
            if ran == 1:
                target.activate_status(move[2].split("-")[0], int(move[2].split("-")[2]))
...
class Player(Monster):
    ...
    def show_moves(self, n, target):
        ...
        while True:
            ...
            for i in range(len(move_set)):
                if "-" in items[i].split("/")[0]:
                    dmg = items[i].split("/")[0].split("-")
                    list_idk = []
                    for n in range(len(dmg)):
                        list_idk.append(str(int(dmg[n]) + self.bd))
                    dmg = "-".join(list_idk)
                    print(f"{move_set[i]} Atk: {dmg} Cost: {items[i].split("/")[1].split("-")[0]} [{i + 
                                                                                                                   1}]")
                else:
                    print(f"{move_set[i]} Atk: {int(items[i].split("/")[0]) + self.bd} Cost: {items[i].split("/")[1].split("-")[0]} [{i + 
                                                                                                                   1}]")

            attack = input()

            if attack.isdigit() and (int(attack) - 1) in range(len(move_set)) and self.will >= int(items[int(attack) - 1].split("/")[1].split("-")[0]):
                print(f"{self.name} used {move_set[int(attack) - 1]}!")

                if len(items[int(attack) - 1].split("/")) == 4:
                    if items[int(attack) - 1].split("/")[3] == "S":
                        self.activate_status(items[int(attack) - 1].split("/")[2], 5)
                        return "c"
                    elif items[int(attack) - 1].split("/")[3].split("-")[0] == "E":
                        ran = random.randint(1, 100)
                        if ran <= int(items[int(attack) - 1].split("/")[3].split("-")[1]):
                            print(f"{self.name} landed a \033[93mcritical\033[0m hit!")
                            self.temp_buff_d += 2
                    elif "M" in items[int(attack) - 1].split("/")[3]:
                        repeat = int(items[int(attack) - 1].split("/")[3].split("-")[1])

                        for i in range(repeat):
                            ran = random.randint(1, 100)

                            if ran <= 75 and target.h > 0:
                ...
                if self.will < 15 and self.type2 == "PALADIN":
                    self.will = 16
                if not items[int(attack) - 1].split("/")[2] == "none":
                    ran = random.randint(1, int(items[int(attack) - 1].split("/")[2].split("-")[1]))
                    if ran == 1:
                        target.activate_status(items[int(attack) - 1].split("/")[2].split("-")[0], int(items[int(attack) - 1].split("/")[2].split("-")[2]))

                return "c"
            elif attack.upper() == "BACK":
                return "b"

            elif attack.isdigit() and self.will <= int(items[int(attack) - 1].split("/")[1].split("-")[0]):
                print("You don't have enough will!")

            else:
                print("Please enter one of the choices.")

    # Spells--
    def show_spells(self, target):
        # set up
        items = []
        ...
        if len(self.spells) > 1:
            for i in range(len(self.spells)):
                for a in range(len(dmg)):
                    if self.spells[i] == dmg[a]:
                        items.append(dmg[a + 1])
        else:
            for a in range(len(dmg)):
                if self.spells[0] == dmg[a]:
                    items.append(dmg[a + 1])
        ...
        while True:
            ...
            for i in range(len(self.spells)):
                if "-" in items[i].split("/")[0]:
                    a = items[i].split("/")[0].split("-")
                    list_idk = []
                    for n in range(len(a)):
                        list_idk.append(str(int(a[n]) + self.bd))
                    a = "-".join(list_idk)
                    print(f"{self.spells[i]} Atk: {a} Cost: {items[i].split("/")[1].split("-")[1]} [{i +
                                                                                                    1}]")
                else:
                    print(
                        f"{self.spells[i]} Atk: {int(items[i].split("/")[0]) + self.bd} Cost: {items[i].split("/")[1].split("-")[1]} [{i +
                                                                                                                                    1}]")

            attack = input()

            if attack.isdigit() and (int(attack) - 1) in range(len(self.spells)) and self.mana >= int(items[int(attack) - 1].split("/")[1].split("-")[1]):
                print(f"{self.name} used {self.spells[int(attack) - 1]}!")

                if len(items[int(attack) - 1].split("/")) > 3:
                    if items[int(attack) - 1].split("/")[3] == "S":
                        self.activate_status(items[int(attack) - 1].split("/")[2], 5)
                        return "c"
                    elif items[int(attack) - 1].split("/")[3].split("-")[0] == "E":
                        ...
                        print(f"{self.name} attacked {int(repeat) + 1} times!")
                        time.sleep(1)
                        ...
                        print(f"{self.name} gathers mana!")
                ...
                if "-" in items[int(attack) - 1].split("/")[0]:
                    ...
                    self.temp_dmg = int(items[int(attack) - 1].split("/")[0])
                self.cal_total_dmg(target)
                self.mana -= int(items[int(attack) - 1].split("/")[1].split("-")[1])
                if not items[int(attack) - 1].split("/")[2] == "none":
                    ran = random.randint(1, int(items[int(attack) - 1].split("/")[2].split("-")[1]))
                    if ran == 1:
                        target.activate_status(items[int(attack) - 1].split("/")[2].split("-")[0], int(items[int(attack) - 1].split("/")[2].split("-")[2]))
            ...
            elif attack.isdigit() and self.mana < int(items[int(attack) - 1].split("/")[1].split("-")[1]):
    ...
    def inventory_add(self, add):
        ...
        print(f"\033[95m{add}\033[0m added to your inventory!")
        ...
        if self.inventory:
            if len(self.inventory) > 0:
                ...
                for i in range(len(self.inventory)):
                    if i == 0:
                        temp_diction[self.inventory[i]] = 1
                    else:
                        create_new = True
                        for n in range(len(temp_diction.keys())):
                            if self.inventory[i] == list(temp_diction.keys())[n]:
                                temp_diction[list(temp_diction.keys())[n]] += 1
                                create_new = False
                ...
                while True:
                    ...
                    for i in range(len(list(temp_diction.keys()))):
                        print(
                            f"[{i + 1}] {list(temp_diction.keys())[i]} - {temp_diction[list(temp_diction.keys())[i]]}x")

                    print()

                    item = input()

                    if item.isdigit() and (int(item) - 1) < len(list(temp_diction.keys())):
                        if self.use_item(self.inventory.index(list(temp_diction.keys())[int(item) - 1])) == "ERROR":
                            print("Something went wrong... This items is unidentifiable.")
                            return "ERROR"
                        self.inventory.pop(self.inventory.index(list(temp_diction.keys())[int(item) - 1]))
                        return "c"
    ...
    def item_effects(self, split):
        ...
        while split[start] != "E":
            ...
            # Adds effects
                ...
                if split[start + 1] == "D":
                    self.bd += int(split[start + 2])
                    start += 1
                elif split[start + 1] == "H":
                    self.mh += int(split[start + 2])
                    self.h += int(split[start + 2])
                    start += 1
                elif split[start + 1] == "C":
                    self.dc += int(split[start + 2])
                    start += 1
                elif split[start + 1] == "S":
                    self.spells.append(split[start + 2])
                    start += 1
                else:
                    print("ERROR")
                    time.sleep(1)
                    print("Perm effect not found!")
                    time.sleep(5)
                    start += 1

            # Random effects
            elif split[start] == "|":
                ran = " ".join(i for i in split if i in ("H", "D", "R", "A"))
                ran = ran.split(" ")
                ...
                for i in range(len(split)):
                    if ran == split[i]:
                        num = i
                if ran == "A":
                    self.item_effects(("A-" + split[num + 1] + "-" + split[num + 2] + "-E"))
                else:
                    self.item_effects((split[num] + "-" + split[num + 1] + "-E"))
            start += 2
            time.sleep(1)

    def reset_survive(self):
        self.survive = 0
